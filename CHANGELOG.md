...
### Added
- **Command monitoring events**: `CommandStartedEvent`, `CommandSucceededEvent`, and `CommandFailedEvent` are now emitted for all CRUD and aggregation operations when subscribers are configured
- `CommandEventSubscriptionBuilder` class for subscribing to command events via `Subscribe<TEvent>(Action<TEvent>)` or `Subscribe(IEventSubscriber)`
- `InMemoryMongoClient` constructor accepting `Action<CommandEventSubscriptionBuilder> commandEventSubscribers`
- `InMemoryMongoOptions.ClusterConfigurator` property for DI configuration via `UseInMemoryMongoDB`
- Supported operations: insert, find, delete, update, findAndModify, aggregate
...
### Fixed
- `$function` test (`Function_CustomExpressionInAddFields`) now uses `$sort` for deterministic ordering, fixing flaky CI failure due to `ConcurrentDictionary` non-deterministic enumeration order
...
### Fixed
- `` with `includeArrayIndex` now correctly returns `null` for scalar (non-array) values instead of `0`
- `` concise correlated syntax (MongoDB 5.0+) now pre-filters foreign docs by `localField`/`foreignField` equality before running the pipeline
- `` update operator now correctly distinguishes missing fields from null fields
...
### Fixed
- `$convert` now correctly converts ObjectId to date by extracting the embedded timestamp
- `$convert` now throws an error when converting non-string values to objectId (was silently returning the input)
- `$convert` now throws an error for unknown target types (was silently returning the input)
...
### Fixed
- `$regexFind` and `$regexFindAll` now correctly populate the `captures` array with matched capture groups (was always returning an empty array)
...
### Fixed
- `$slice` expression now throws an error when count is negative and position is specified (was silently returning empty array)
- `$project` aggregation stage now throws an error when mixing field inclusions and exclusions (was silently accepting)
...
### Fixed
- `$stdDevPop` and `$stdDevSamp` accumulators now return `Decimal128` when any input value is `Decimal128` (was always returning `Double`)
...
### Fixed
- Aggregation `$add`, `$subtract`, `$multiply`, `$mod` now return `Decimal128` when any operand is Decimal128 (was incorrectly returning Double)
- `$dateDiff` with `unit: "week"` now correctly counts weekly boundaries crossed instead of naive division by 7
- `$dateDiff` with `unit: "quarter"` now correctly counts quarter boundaries crossed (Jan/Apr/Jul/Oct) instead of naive month-diff/3
- `$dateDiff` now supports `startOfWeek` parameter for week boundary counting
